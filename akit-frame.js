(function () {
  ...
  class AKIT_SceneBasic {
    constructor(options) {
      ...
      if (AKIT.canvas == undefined || AKIT.canvas == 0) {
        ...
        AKIT.canvas.height = this.SCREEN_HEIGHT;
        AKIT.canvas.width = this.SCREEN_WIDTH; //  console.log(AKIT.canvas,this.SCREEN_HEIGHT,this.SCREEN_WIDTH);
      }
  ...
  */
  ...
  class AKIT_ObjectTypeSphereDebug extends AKIT_ObjectType {
  ...
  class AKIT_AppLoader {
    ...
    add(name, args) {
      ...
      if (name == 'test') {
        let s = args.size;
        l = new AKIT_ObjectTypeSphereDebug(s, s, s);
        readyNow = true;
      }
  ...
  */
  ...
  class AKIT_SceneInjected {
    constructor(options, scene, renderer, camera) {
      ...
      this.sceneType = APP.sceneType;
      this.windowLoaded = false;
      if (AKIT.canvas == undefined) AKIT.canvas = document.createElement('canvas');
  ...
  class AKIT_8Wall {
    ...
    frame() {//  this.pointsUpdate();
    }

    resize() {
      var screenWidth = window.innerWidth;
      var screenHeight = window.innerHeight; //var sourceWidth = this.domElement.videoWidth;
    ...
    //    for (var i = 0; i < this.pointsX; i++) {
    //    var idx = j * this.pointsX + i;
    //    var u = i / this.pointsX;
    //    var v = j / this.pointsY;
    //  var ht = XR.XrController.hitTest(u, v, ['FEATURE_POINT'])
    //    console.log(idx,u,v,ht)
    //    this.points[idx]=ht;
    //    }
    //  }
    ...
    run() {
      console.log('8Wall run'); //   XR.XrController.configure({ enableWorldPoints: true });
      ...
      //    XRExtras.RuntimeError.pipelineModule(), // Shows an error image on runtime error.
      {
        ...
        onStart: ({
          ...
          AKIT.scene.init(); //  AKIT.scene.scene = threeObj.scene;
          //  AKIT.scene.camera = threeObj.camera;

          console.log('8Wall set', APP.renderer, AKIT.scene, AKIT.scene.camera, this);
  ...
  class AKIT_ARJSToolkitSource {
    ...
    onResizeElement() {
      ...
      var screenWidth = AKIT.windowWidth; // window.innerWidth; //AKIT.windowWidth; //window.innerWidth;

      var screenHeight = AKIT.windowHeight; //window.innerHeight; // //window.innerHeight;
      //var sourceWidth = 640; //this.domElement.videoWidth;
      //var sourceHeight = 480; // this.domElement.videoHeight;

      var sourceWidth = this.domElement.videoWidth;
      var sourceHeight = this.domElement.videoHeight; //var sourceWidth = 480; //this.domElement.videoWidth;
      //var sourceHeight = 640; // this.domElement.videoHeight;

      console.log(screenWidth, screenHeight, sourceWidth, sourceHeight, this.domElement, AKIT.canvas); // compute sourceAspect
      ...
      if (screenAspect < sourceAspect) {
        ...
        console.log('<', screenAspect, sourceAspect, newWidth, this.domElement.style.width, this.domElement.style.height, this.domElement, APP.renderer.domElement);
      } else {
        ...
        console.log('>', screenAspect, sourceAspect, this.domElement.style.width, this.domElement.style.height);
      }
      /*  console.log(
        'sizes',
    ...
    copyElementSizeTo(otherElement) {
      console.log('copy element', AKIT.windowWidth, AKIT.windowHeight, otherElement);

      if (AKIT.windowWidth > AKIT.windowHeight) {
        //landscape
        otherElement.style.width = this.domElement.style.width;
        otherElement.style.height = this.domElement.style.height;
        otherElement.style.marginLeft = this.domElement.style.marginLeft;
        otherElement.style.marginTop = this.domElement.style.marginTop; //  console.log(
        //    'lands',
        //   otherElement.style.width,
        //    this.domElement.style.width,
        //   otherElement.style.height,
        //   this.domElement.style.height
        // );
      } else {
        //portrait
        otherElement.style.height = this.domElement.style.height; //   console.log(otherElement.style.height)

        otherElement.style.width = parseInt(this.domElement.style.height) * 4 / 3 + 'px'; //   console.log(otherElement.style.width)

        otherElement.style.marginLeft = (AKIT.windowWidth - parseInt(otherElement.style.width)) / 2 + 'px';
        otherElement.style.marginTop = 0; //  console.log('port0',otherElement.style.width,this.domElement.style.height,(parseInt(this.domElement.style.height) * 4) / 3 );
        // this.domElement.style.width =  otherElement.style.width
        //  console.log(
        //    'port',
        //   otherElement.style.width,
        //   this.domElement.style.width,
  ...
  class AKIT_ObjectARSource {
    ...
    onResize() {
      // console.log('resize0',APP.renderer,APP.renderer.domElement.width,APP.renderer.domElement.style)
      //  console.log('resize start');
      //    console.log('resize 0', APP.renderer.domElement,AKIT.canvas,   this.properties.context.arController.canvas,AKIT.canvas.height, AKIT.canvas.width);
      this.properties.arsource.onResizeElement();

      if (this.copyElementTo) {
        this.properties.arsource.copyElementSizeTo(APP.renderer.domElement);
  ...
  class AKIT_ARJSScene {
    constructor(sourceType, cameraParametersUrl, detectionMode) {
      ...
      this.source = new AKIT_ObjectARSource(this.properties.sourceType, this.context.properties.arcontext);
  ...
  class AKIT_ARAppAR {
    ...
    init8Wall() {
      ...
      window.onload = () => {
        AKIT.loadedAR = true; //window.XRExtras ? load() : window.addEventListener('xrextrasloaded', load);
      };

      loadJS('//apps.8thwall.com/xrweb?appKey=02WuZznVRhFTavwaiH0zni8rffOx6WTTCzh6QuMl3AveIKlUctiQHeLnzCi48pCbIaJPyM', document.body);
    ...
    match() {
      ...
      for (var i = 0; i < matches.length; i++) {
        if (this.debugObjs[i] == undefined) {
          this.debugObjs[i] = new AKIT_ObjectTypeSphereDebug(0.01, 4, 4);
  ...
  */
  ...
  class APP_start {
    ...
    properties() {
      ...
      if (APP.platformAR == 'arjs') ; //  APP.arjs.cameraParametersUrl = AKIT_PROPS.arjs.cameraParametersUrl
      //  APP.arjs.cameraParametersUrl: '/assets/platform/arjs/camera_para.dat',


      console.log('akit-aframe (properties):', AKIT_PROPS, APP);
