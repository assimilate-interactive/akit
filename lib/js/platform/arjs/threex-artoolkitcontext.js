...
ARjs.Context = THREE.ArToolkitContext = function(parameters) {
  var _this = this;
  ...
  console.assert(
    ['artoolkit', 'aruco', 'tango'].indexOf(this.parameters.trackingBackend) !== -1,
    'invalid parameter trackingBackend',
    this.parameters.trackingBackend
  );
  console.assert(
    ['color', 'color_and_matrix', 'mono', 'mono_and_matrix'].indexOf(this.parameters.detectionMode) !== -1,
  ...
  function setParameters(parameters) {
    ...
    for (var key in parameters) {
      ...
      if (newValue === undefined) {
        console.warn("THREE.ArToolkitContext: '" + key + "' parameter is undefined.");
        continue;
      }
      ...
      if (currentValue === undefined) {
        console.warn("THREE.ArToolkitContext: '" + key + "' is not a property of this material.");
        continue;
      }
...
ARjs.Context.prototype.addMarker = function(arMarkerControls) {
  //	console.assert(arMarkerControls instanceof THREE.ArMarkerControls)
...
ARjs.Context.prototype._initAruco = function(onCompleted) {
  this.arucoContext = new THREE.ArucoContext();
